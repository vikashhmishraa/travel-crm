    ...
    <div class="col-md-3" id="sidebar" >
        ...
        <div class="box box-primary">
            <div class="box-body box-profile">
                ...
                <p class="text-muted text-center text-yellow">{{ SiteHelpers::formatLookUp($row->hotelcategoryID,'hotelcategoryID','1:def_hotel_categories:hotelcategoryID:category_name') }}</p>
                ...
                <ul class="list-group list-group-unbordered">
                    ...
                    <li class="list-group-item">
                        <b>{{ Lang::get('core.type') }}</b> <a href="{{ url('hotels?search=hoteltypeID:=:'.$row->hoteltypeID) }}" class="pull-right" target="_blank">{{ SiteHelpers::formatLookUp($row->hoteltypeID,'hoteltypeID','1:def_hotel_type:hoteltypeID:type_name') }}</a>
                    </li>
                    <li class="list-group-item">
                    ...
                    <li class="list-group-item">
                        <b>{{ Lang::get('core.status') }}</b> <a class="pull-right">{!! GeneralStatus::Status($row->status) !!}</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                <div class="tab-pane active" id="description">
                    ...
                    <p class="text-muted">{{ $row->address}}<br>{{ SiteHelpers::formatLookUp($row->cityID,'cityID','1:def_city:cityID:city_name') }}, {{ SiteHelpers::formatLookUp($row->countryID,'countryID','1:def_country:countryID:country_name') }}</p>
                    ...
                    <p>{!! SiteHelpers::showPaymentOptions($row->paymentoptions) !!}</p>
                    <hr>
                    ...
                    <hr>
                    <strong>
                    {{ Lang::get('core.checkin') }} :</strong> {{ SiteHelpers::formatLookUp($row->checkin,'checkin','1:def_time_slots:timeslotID:time') }} <strong>{{ Lang::get('core.checkout') }} :</strong>{{ SiteHelpers::formatLookUp($row->checkout,'checkout','1:def_time_slots:timeslotID:time') }}
                    <hr>
                ...
                <div class="tab-pane" id="RoomRates">
                    ...
                    <table id="notes" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach($room as $rr)
                            <tr>
                                <td>{{ SiteHelpers::formatLookUp($rr['roomtypeID'],'roomtypeID','1:def_room_types:roomtypeID:room_type') }}</td>
                                <td>@if ($rr['season'] == 0) Low Season @elseif ($rr['season'] == 1) High Season @endif </td>
                                <td>{{ SiteHelpers::formatLookUp($rr['currency'],'currency','1:def_currency:currencyID:currency_sym|symbol') }} {{ $rr['rate'] }}</td>
                    ...
                    <table id="hotelnotes" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $hn['title'] }}</td>
                                <td>{{ $hn['note'] }}</td>
                                <td>{{ SiteHelpers::TarihFormat($hn['created_at'])}}</td>
                                <td>@if($access['is_remove'] =='1')
