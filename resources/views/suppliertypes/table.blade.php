...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'suppliertypes/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->suppliertypeID;
           		?>
                <tr class="editable" id="form-{{ $row->suppliertypeID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->suppliertypeID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->suppliertypeID }}" data-url="{{ url('suppliertypes/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->suppliertypeID ;?>"  >
					{!! AjaxHelpers::buttonAction('suppliertypes',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->suppliertypeID,'suppliertypeID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->suppliertypeID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
