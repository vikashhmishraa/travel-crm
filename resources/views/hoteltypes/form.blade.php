...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'hoteltypes/save/'.SiteHelpers::encryptID($row['hoteltypeID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'hoteltypesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.hoteltypes') }}</legend>
				{!! Form::hidden('hoteltypeID', $row['hoteltypeID']) !!}					
									  <div class="form-group  " >
										<label for="Type Name" class=" control-label col-md-4 text-left"> {{ Lang::get('core.hoteltype') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='type_name' id='type_name' value='{{ $row['type_name'] }}' 
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("hoteltypes/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
