...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'roomtypes/save/'.SiteHelpers::encryptID($row['roomtypeID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'roomtypesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.roomtypes') }}</legend>
				{!! Form::hidden('roomtypeID', $row['roomtypeID']) !!}					
									  <div class="form-group  " >
										<label for="Room Type" class=" control-label col-md-4 text-left"> {{ Lang::get('core.roomtype') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='room_type' id='room_type' value='{{ $row['room_type'] }}' 
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("roomtypes/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
