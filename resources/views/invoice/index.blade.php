        ...
        <tbody>
            @foreach ($rowData as $row)
                <tr>
                    ...
                    <td><a href="{{ url('createbooking/show/'.$row->bookingID.'?return='.$return) }}" target="_blank">{{ SiteHelpers::formatLookUp($row->bookingID,'bookingID','1:bookings:bookingsID:bookingno') }}</a>
                    </td>
                    <td>{{ $row->InvTotal}} {{ SiteHelpers::formatLookUp($row->currency,'currencyID','1:def_currency:currencyID:currency_sym') }}
                    </td>
                    <td><a href="{{ url('travellers/show/'.$row->travellerID.'?return='.$return) }}" target="_blank">{{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:nameandsurname') }}</a>
                    </td>
