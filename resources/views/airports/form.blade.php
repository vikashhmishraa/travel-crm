...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'airports/save/'.SiteHelpers::encryptID($row['airportID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'airportsFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.airports') }}</legend>
				{!! Form::hidden('airportID', $row['airportID']) !!}					
									  <div class="form-group  " >
										<label for="Airport Name" class=" control-label col-md-4 text-left"> {{ Lang::get('core.airport') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='airport_name' id='airport_name' value='{{ $row['airport_name'] }}' 
						required     class='form-control ' /> 
                            ...
                            <div class="form-group  " >
										<label for="IATA" class=" control-label col-md-4 text-left"> {{ Lang::get('core.IATA') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='IATA' id='IATA' value='{{ $row['IATA'] }}' 
						required     class='form-control ' /> 
										 </div> 
										 <div class="col-md-2">
...
$(document).ready(function() { 
		...
		$("#countryID").jCombo("{!! url('airports/comboselect?filter=def_country:countryID:country_name') !!}",
		{  selected_value : '{{ $row["countryID"] }}' });
		
		$("#cityID").jCombo("{!! url('airports/comboselect?filter=def_city:cityID:city_name') !!}&parent=countryID:",
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("airports/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
