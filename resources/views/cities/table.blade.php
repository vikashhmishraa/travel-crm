			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->cityID;
           		?>
                <tr class="editable" id="form-{{ $row->cityID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->cityID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->cityID }}" data-url="{{ url('cities/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->cityID ;?>"  >
					{!! AjaxHelpers::buttonAction('cities',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->cityID,'cityID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->cityID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
