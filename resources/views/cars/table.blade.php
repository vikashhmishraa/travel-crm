    ...
    <table class="table table-bordered table-striped " id="{{ $pageModule }}Table">
        <thead>
			<tr>
				...
				<th>{{Lang::get('core.featured')}}</th>	
				<th>{{Lang::get('core.model')}}</th>	
				<th>{{Lang::get('core.passengers')}}</th>	
				<th>{{Lang::get('core.type')}}</th>	
				<th>{{Lang::get('core.dailyrate')}}</th>	
				<th>{{Lang::get('core.weeklyrate')}}</th>	
				<th>{{Lang::get('core.monthlyrate')}}</th>	
				<th>{{Lang::get('core.currency')}}</th>	
				<th>{{Lang::get('core.airportpickup')}}</th>	
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->carsID;
           		?>
                <tr class="editable" id="form-{{ $row->carsID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->carsID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->carsID }}" data-url="{{ url('cars/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->carsID ;?>"  >
					{!! AjaxHelpers::buttonAction('cars',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->carsID,'carsID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->carsID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
