...
<h3 class="text-center">          
              ...
              <ul class="list-group list-group-unbordered">
                ...
                <li class="list-group-item">
                  ...
                  <li class="list-group-item">
                  <b>{{Lang::get('core.city')}}</b> <a class="pull-right">{{ SiteHelpers::formatLookUp($row->cityID,'cityID','1:def_city:cityID:city_name') }}</a>
                </li>
                  <li class="list-group-item">
                  <b>{{Lang::get('core.country')}}</b> <a class="pull-right">{{ SiteHelpers::formatLookUp($row->countryID,'countryID','1:def_country:countryID:country_name') }}</a>
                </li>
                  <li class="list-group-item">
...
<table id="guidetours" class="table table-bordered table-striped">
             ...
             @foreach($guide as $gt)
                <tr>
                  <td><a href="{{ url('tourdates/show/'.$gt['tourdateID']) }}" target="_blank">{{ SiteHelpers::formatLookUp($gt['tourID'],'tourID','1:tours:tourID:tour_name') }}</a></td>
                  <td>{{ SiteHelpers::TarihFormat($gt['start'])}}</td>
                  <td>{{ SiteHelpers::TarihFormat($gt['end'])}}</td>
                  <td>{!! SiteHelpers::Featured($gt['featured']) !!}</td>
                  <td>{!! SiteHelpers::Definite_departure($gt['definite_departure']) !!}</td>
                  <td>{!! GeneralStatus::Status($gt['status']) !!}</td>
                </tr>
            @endforeach
                </tbody>
              </table>                  
             ...
             @foreach($notes as $gn)
                <tr>
                  ...
                  <td>{{ $gn['title'] }}</td>
                  <td>{{ $gn['note'] }}</td>
                  <td>{{ SiteHelpers::TarihFormat($gn['created_at'])}}</td>
