...
<b>{{Lang::get('core.name')}}:</b> {{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:nameandsurname') }}<br>
<b>{{Lang::get('core.address')}}:</b><br>
{{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:address') }} {{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:city') }} 
<?php $invocountry=SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:countryID') ?>
{{ SiteHelpers::formatLookUp($invocountry,'countryID','1:def_country:countryID:country_name') }} <br>
<b>{{Lang::get('core.email')}}:</b> {{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:email') }}<br>
<b>{{Lang::get('core.tel')}}:</b>{{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:phone') }}<br>
</p>
            <table  class="maintable" cellpadding="0" cellspacing="0">
             ...
             @foreach($rms as $rs)
                @if($rs['status']==1)
                <tr>
                  <td align="left">{!! SiteHelpers::roomType2($rs['roomtype']) !!}</td>
                  <td align="left">{!! SiteHelpers::showTravellers2($rs['travellers']) !!}</td>
                  <td align="left" >{!! BookingStatus::Status($rs['status']) !!}
                    </td>
                </tr>
                @endif
            @endforeach
        ...
        @if ($trs != NULL)
             ...
             @foreach($trs as $ts)
                @if($ts['status']==1)
                <tr>
                  <td align="left"><a href="{{ url('tours/show/'.$ts['tourID'])}}" target="_blank"><strong>{{ SiteHelpers::formatLookUp($ts['tourID'],'tourID','1:tours:tourID:tour_name') }}</strong></a></td>
                  <td align="left"><a href="{{ url('tourdates/show/'.$ts['tourdateID'])}}" target="_blank"><strong> {{ SiteHelpers::formatLookUp($ts['tourdateID'],'tourdateID','1:tour_date:tourdateID:tour_code') }}</strong></a></td>
                  <td align="left">{{date("".CNF_DATE."",strtotime(SiteHelpers::formatLookUp($ts['tourdateID'],'tourdateID','1:tour_date:tourdateID:start'))) }}</td>
                <td align="left">{{ date("".CNF_DATE."",strtotime(SiteHelpers::formatLookUp($ts['tourdateID'],'tourdateID','1:tour_date:tourdateID:end'))) }}</td>
                  <td align="left"  >{!! BookingStatus::Status($ts['status']) !!}
                  </td>
                </tr>
                @endif
            @endforeach
            ...
            <table  class="maintable"  cellpadding="0" cellspacing="0">
             ...
             @foreach($hotel as $hot)
                ...
                <tr>
                  ...
                  <td>{!! BookingStatus::Status($hot['status']) !!}</td>
                </tr>
                @endif
            @endforeach
                </tbody>
...
@if ( $flight !=NULL )
      <table  class="maintable"  cellpadding="0" cellspacing="0">
             ...
             @foreach($flight as $fl)
                @if($fl['status']==1)
                <tr>
                  <td>{!! SiteHelpers::showTravellers2($fl['travellersID']) !!} </td>
                  <td>{{ SiteHelpers::formatLookUp($fl['airlineID'],'airlineID','1:def_airlines:airlineID:airline') }}</td>
                  <td>{{ SiteHelpers::formatLookUp($fl['depairportID'],'depairportID','1:def_airports:airportID:IATA') }}-{{ SiteHelpers::formatLookUp($fl['arrairportID'],'depairportID','1:def_airports:airportID:IATA') }}</td>
                  <td>{{ SiteHelpers::TarihFormat2($fl['departing'])}}</td>
                  <td>{{ SiteHelpers::TarihFormat2($fl['returning'])}}</td>
                  <td>{!! BookingStatus::Status($fl['status']) !!}</td>
                </tr>
                @endif
            @endforeach
                </tbody>
...
@if ( $car != NULL )
      <table  class="maintable"  cellpadding="0" cellspacing="0">
             ...
             @foreach($car as $cr)
                ...
                <td>{{ $cr['pickup'] }}</td>
                <td>{{ $cr['dropoff'] }}</td>
                <td >{!! BookingStatus::Status($cr['status']) !!}</td>
                </tr>
                @endif
            @endforeach
                </tbody>
...
@if ( $extra != NULL )
      <table  class="maintable"  cellpadding="0" cellspacing="0">
             ...
             @foreach($extra as $ex)
                ...
                <td  >{!! BookingStatus::Status($ex['status']) !!}</td>
                </tr>
                @endif
            @endforeach
                </tbody>
