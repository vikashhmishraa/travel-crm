...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'guidelanguages/save/'.SiteHelpers::encryptID($row['languageID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'guidelanguagesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{Lang::get('core.guidelanguages')}}</legend>
				{!! Form::hidden('languageID', $row['languageID']) !!}					
									  <div class="form-group  " >
										<label for="Language" class=" control-label col-md-4 text-left"> {{Lang::get('core.language')}} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='language_name' id='language_name' value='{{ $row['language_name'] }}' 
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("guidelanguages/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
	...
	var form = $('#guidelanguagesFormAjax'); 
	form.parsley();
