			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->commentID }}">
				 ...
				 <td data-values="action" data-key="<?php echo $row->commentID ;?>"  >
					{!! AjaxHelpers::buttonAction('commentscheck',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->commentID,'commentID') !!}	
						
				</td>		
                    <td><a href="" target="_blank">{{ SiteHelpers::formatLookUp($row->pageID,'pageID','1:tb_pages:pageID:title') }}</a></td>
                    <td>{{ SiteHelpers::formatLookUp($row->userID,'id','1:tb_users:id:username') }}</td>
                    <td>{{ $row->comments }}</td>
                    <td>{{ SiteHelpers::TarihFormat($row->posted) }}</td>
