...
{{ Lang::get('core.nobookingmade') }}
                ...
                <tr>
<td align="left" colspan="6">{{ Lang::get('core.tourname') }}: {{ SiteHelpers::formatLookUp($row->tourID,'tourID','1:tours:tourID:tour_name') }}<br>{{ Lang::get('core.tourdate') }}: {{ SiteHelpers::TarihFormat($row->start)}} / {{ SiteHelpers::TarihFormat($row->end)}}<br>{{ Lang::get('core.tourcode') }}:{{ $row->tour_code}}<br>{{ Lang::get('core.tourcode') }}: {{ SiteHelpers::formatLookUp($row->guideID,'guideID','1:guides:guideID:name') }}</td>
