...
<div class="box box-primary col-md-12">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'bookhotel/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->bookhotelID;
           		?>
                <tr class="editable" id="form-{{ $row->bookhotelID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->bookhotelID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->bookhotelID }}" data-url="{{ url('bookhotel/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->bookhotelID ;?>"  >
					{!! AjaxHelpers::buttonAction('bookhotel',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->bookhotelID,'bookhotelID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->bookhotelID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
