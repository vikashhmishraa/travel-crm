		...
		<table class="table table-striped table-bordered" >
			<tbody>	
					...
					<tr>
						...
						<td>{{ SiteHelpers::TarihFormat($row->applicationdate) }} </td>
					...
					<tr>
						...
						<td>{{ $row->payment}} {{ SiteHelpers::formatLookUp($row->currencyID,'currencyID','1:def_currency:currencyID:currency_name') }}</td>
						
					</tr>				
					<tr>
						<td width='40%' class='label-view text-right'><strong>Payment Type</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->paymenttypeID,'paymenttypeID','1:def_payment_types:paymenttypeID:payment_type') }} </td>
						
					</tr>
				
					<tr>
						<td width='40%' class='label-view text-right'><strong>Application Fee</strong></td>
						<td>{{ $row->applicationfee}} {{ SiteHelpers::formatLookUp($row->currencyID2,'currencyID2','1:def_currency:currencyID:currency_name') }}</td>
                    ...
                    <tr>
						<td width='40%' class='label-view text-right'><strong>{{ SiteHelpers::activeLang('Remarks', (isset($fields['remarks']['language'])? $fields['remarks']['language'] : array())) }}</strong></td>
						<td>{{ $row->remarks}} </td>
					...
					<tr>
						...
						<td><span>{!! GeneralStatus::Visa($row->status) !!}</span> </td>
						
					</tr>
				@if($row->status=='3')
					<tr>
						<td width='40%' class='label-view text-right'><strong>Visa Expiry Date</strong></td>
						<td>{{ SiteHelpers::TarihFormat($row->visaexpirydate) }} </td>
