...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'optionaltours/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
    ...
    <table class="table table-bordered table-striped " id="{{ $pageModule }}Table">
        <thead>
			<tr>
                ...
                <th >{{Lang::get('core.optionaltour')}}</th>	
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->optionaltourID;
           		?>
                <tr class="editable" id="form-{{ $row->optionaltourID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->optionaltourID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->optionaltourID }}" data-url="{{ url('optionaltours/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->optionaltourID ;?>"  >
					{!! AjaxHelpers::buttonAction('optionaltours',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->optionaltourID,'optionaltourID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->optionaltourID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
