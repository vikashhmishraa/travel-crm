...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'optionaltours/save/'.SiteHelpers::encryptID($row['optionaltourID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'optionaltoursFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.optionaltours') }}</legend>
				{!! Form::hidden('optionaltourID', $row['optionaltourID']) !!}					
									  <div class="form-group  " >
										<label for="Optional Tour" class=" control-label col-md-4 text-left"> {{ Lang::get('core.optionaltour') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='optional_tour' id='optional_tour' value='{{ $row['optional_tour'] }}' 
...
$(document).ready(function() { 
		...
		$("#currencyID").jCombo("{!! url('optionaltours/comboselect?filter=def_currency:currencyID:currency_sym|symbol&limit=WHERE:status:=:1') !!}",
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("optionaltours/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
