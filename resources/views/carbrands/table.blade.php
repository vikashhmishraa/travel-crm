...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 <?php echo Form::open(array('url'=>'carbrands/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->carbrandID;
           		?>
                <tr class="editable" id="form-{{ $row->carbrandID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->carbrandID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->carbrandID }}" data-url="{{ url('carbrands/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->carbrandID ;?>"  >
					{!! AjaxHelpers::buttonAction('carbrands',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->carbrandID,'carbrandID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->carbrandID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
