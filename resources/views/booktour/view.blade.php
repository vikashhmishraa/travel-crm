...
@endif	
		...
		<table class="table  table-bordered" >
			<tbody>	
					...
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{ SiteHelpers::activeLang('Booking #', (isset($fields['bookingID']['language'])? $fields['bookingID']['language'] : array())) }}</strong></td>
						<td><a href="createbooking/show/{{$row->bookingID}}">{{ $row->bookingID}} </a> </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{ SiteHelpers::activeLang('Tour', (isset($fields['tourID']['language'])? $fields['tourID']['language'] : array())) }}</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->tourID,'tourID','1:tours:tourID:tour_name') }} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{ SiteHelpers::activeLang('Tour Date', (isset($fields['tourdateID']['language'])? $fields['tourdateID']['language'] : array())) }}</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->tourdateID,'tourdateID','1:tour_date:tourdateID:start|end') }} </td>
