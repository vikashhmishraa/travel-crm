            ...
            <table class="table table-bordered table-striped " id="{{ $pageModule }}Table">
                ...
                <tbody>
                    ...
                    <tr id="form-0">
                        ...
                        <?php $limited = isset($t['limited']) ? $t['limited'] :''; ?> @if(SiteHelpers::filterColumn($limited ))
           		...
           		?>
                    <tr class="editable" id="form-{{ $row->booktourID }}">
                        ...
                        <td data-values="action" data-key="<?php echo $row->booktourID ;?>">
                            {!! AjaxHelpers::buttonAction('booktour',$access,$id ,$setting) !!} {!! AjaxHelpers::buttonActionInline($row->booktourID,'booktourID') !!}
                        ...
                        <?php foreach ($tableGrid as $field) :
					 	if($field['view'] =='1') : 
							$value = SiteHelpers::formatRows($row->{$field['field']}, $field , $row);
						 	?>
                        <?php $limited = isset($field['limited']) ? $field['limited'] :''; ?> @if(SiteHelpers::filterColumn($limited ))
