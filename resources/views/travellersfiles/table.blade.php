			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->fileID }}">
				 ...
				 <td data-values="action" data-key="<?php echo $row->fileID ;?>"  >
					{!! AjaxHelpers::buttonAction('travellersfiles',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->fileID,'fileID') !!}	
					 ...
					 <td>{!! SiteHelpers::showUploadedFile($row->file,'/uploads/images/') !!}</td>
					 <td>{{$row->remarks }}</td>
