...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'roomamenities/save/'.SiteHelpers::encryptID($row['roomamenityID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'roomamenitiesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.roomamenities') }}</legend>
				{!! Form::hidden('roomamenityID', $row['roomamenityID']) !!}					
									  <div class="form-group  " >
										<label for="Room Amenty" class=" control-label col-md-4 text-left"> {{ Lang::get('core.roomamenity') }}<span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='amenty' id='amenty' value='{{ $row['amenty'] }}' 
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("roomamenities/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
	...
	var form = $('#roomamenitiesFormAjax'); 
	form.parsley();
