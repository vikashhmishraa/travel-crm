...
<div class="box box-primary col-md-12">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'bookflight/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->bookflightID }}">
					...
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->bookflightID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->bookflightID }}" data-url="{{ url('bookflight/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->bookflightID ;?>"  >
					{!! AjaxHelpers::buttonAction('bookflight',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->bookflightID,'bookflightID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->bookflightID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
