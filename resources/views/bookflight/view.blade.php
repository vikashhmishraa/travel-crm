		...
		<table class="table  table-bordered" >
			<tbody>	
					...
					<tr>
						...
						<td><a href="createbooking/show/{{$row->bookingID}}">{{ SiteHelpers::formatLookUp($row->bookingID,'bookingID','1:bookings:bookingsID:bookingno') }} </a> </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.roundtrip')}}</strong></td>
						<td>{!! SiteHelpers::formatRows($row->return,$fields['return'],$row ) !!} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.travellers')}}</strong></td>
						<td>{!! SiteHelpers::showTravellers($row->travellersID) !!} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.departureairport')}}</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->depairportID,'depairportID','1:def_airports:airportID:airport_name') }} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.arrivalairport')}}</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->arrairportID,'arrairportID','1:def_airports:airportID:airport_name') }} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.departuredate')}}</strong></td>
						<td>{{ SiteHelpers::TarihFormat($row->departing)}} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong>{{Lang::get('core.returndate')}}</strong></td>
						<td>{{ SiteHelpers::TarihFormat($row->returning)}} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong> {{Lang::get('core.airline')}}</strong></td>
						<td>{{ SiteHelpers::formatLookUp($row->airlineID,'airlineID','1:def_airlines:airlineID:airline') }} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'><strong> {{Lang::get('core.class')}} </strong></td>
						<td>{!! SiteHelpers::formatRows($row->class,$fields['class'],$row ) !!} </td>
					...
					<tr>
						...
						<td>{!! BookingStatus::Status($row->status) !!} </td>
