...
<div class="box box-primary col-md-12">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'bookroom/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->roomID;
           		?>
                <tr class="editable" id="form-{{ $row->roomID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->roomID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->roomID }}" data-url="{{ url('bookroom/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->roomID ;?>"  >
					{!! AjaxHelpers::buttonAction('bookroom',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->roomID,'roomID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->roomID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
