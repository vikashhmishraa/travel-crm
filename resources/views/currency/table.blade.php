...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'currency/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
    ...
    <table class="table table-bordered table-striped " id="{{ $pageModule }}Table">
        <thead>
			<tr>
				...
				<th>{{Lang::get('core.acronym')}}</th>	
				<th>{{Lang::get('core.symbol')}}</th>	
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->currencyID;
           		?>
                <tr class="editable" id="form-{{ $row->currencyID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->currencyID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->currencyID }}" data-url="{{ url('currency/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->currencyID ;?>"  >
					{!! AjaxHelpers::buttonAction('currency',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->currencyID,'currencyID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->currencyID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
