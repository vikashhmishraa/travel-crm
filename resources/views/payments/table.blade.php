			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->invoicePaymentID }}">
				 ...
				 <td data-values="action" data-key="<?php echo $row->invoicePaymentID ;?>"  >
					{!! AjaxHelpers::buttonAction('payments',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->invoicePaymentID,'invoicePaymentID') !!}	
						
				</td>						
                    <td><a href="{{ url('travellers/show/'.$row->travellerID.'?return='.$return)}}" target="_blank">{{ SiteHelpers::formatLookUp($row->travellerID,'travellerID','1:travellers:travellerID:nameandsurname') }}</a></td>
                    <td><a href="{{ url('invoice/show/'.$row->invoiceID.'?return='.$return)}}" target="_blank">{{$row->invoiceID}}</a></td>
					 <td>{{$row->amount}} {{ SiteHelpers::formatLookUp($row->currency,'currencyID','1:def_currency:currencyID:currency_sym') }}</td>
					 <td> {{ SiteHelpers::formatLookUp($row->payment_type,'paymenttypeID','1:def_payment_types:paymenttypeID:payment_type') }}</td>
					 <td>{{ SiteHelpers::TarihFormat($row->payment_date)}}</td>
