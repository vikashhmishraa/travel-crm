...
<div class="box box-primary">
	...
	<div class="box-body">
	 ...
	 {!! Form::open(array('url'=>'tandc/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable' )) !!}
            ...
            @foreach ($rowData as $row)
                <tr>
					...
					<td>
							...
							<a href="{{ url('tandc/show/'.$row->tandcID.'?return='.$return)}}" class="tips" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search fa-2x"></i> </a>
							@endif
							@if($access['is_edit'] ==1)
							<a  href="{{ url('tandc/update/'.$row->tandcID.'?return='.$return) }}" class="tips" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit fa-2x"></i> </a>
							@endif
...
$(document).ready(function(){
	...
	$('.do-quick-search').click(function(){
		$('#MmbTable').attr('action','{{ url("tandc/multisearch")}}');
		$('#MmbTable').submit();
	});
	...
	$('.copy').click(function() {
		...
		{
				$('#MmbTable').attr('action','{{ url("tandc/copy")}}');
