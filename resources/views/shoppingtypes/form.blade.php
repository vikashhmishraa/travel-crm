...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'shoppingtypes/save/'.SiteHelpers::encryptID($row['shoppingtypeID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'shoppingtypesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.shoppingtypes') }}</legend>
				{!! Form::hidden('shoppingtypeID', $row['shoppingtypeID']) !!}					
									  <div class="form-group  " >
										<label for="Shopping Type" class=" control-label col-md-4 text-left"> {{ Lang::get('core.shoppingtype') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='shopping_type' id='shopping_type' value='{{ $row['shopping_type'] }}' 
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("shoppingtypes/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
	...
	var form = $('#shoppingtypesFormAjax'); 
	form.parsley();
