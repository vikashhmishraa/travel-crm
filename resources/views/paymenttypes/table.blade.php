...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'paymenttypes/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
        ...
        <tbody>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		?>
                <tr class="editable" id="form-{{ $row->paymenttypeID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->paymenttypeID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->paymenttypeID }}" data-url="{{ url('paymenttypes/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->paymenttypeID ;?>"  >
					{!! AjaxHelpers::buttonAction('paymenttypes',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->paymenttypeID,'paymenttypeID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->paymenttypeID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
