...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'countries/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
    ...
    <table class="table table-bordered table-striped " id="{{ $pageModule }}Table">
        <thead>
			<tr>
				...
				<th>{{Lang::get('core.countrycode')}}</th>	
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->countryID;
           		?>
                <tr class="editable" id="form-{{ $row->countryID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->countryID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->countryID }}" data-url="{{ url('countries/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->countryID ;?>"  >
					{!! AjaxHelpers::buttonAction('countries',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->countryID,'countryID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->countryID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
