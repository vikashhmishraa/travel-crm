...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'countries/save/'.SiteHelpers::encryptID($row['countryID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'countriesFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.countries') }}</legend>
				{!! Form::hidden('countryID', $row['countryID']) !!}					
						...
						required     class='form-control ' /> 
									  ...
									  <div class="form-group  " >
										<label for="Country Code" class=" control-label col-md-4 text-left"> {{ Lang::get('core.countrycode') }} <span class="asterix"> * </span></label>
										<div class="col-md-6">
										  <input  type='text' name='country_code' id='country_code' value='{{ $row['country_code'] }}' 
						required     class='form-control ' /> 
										 </div> 
										 <div class="col-md-2">
...
$(document).ready(function() { 
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("countries/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
