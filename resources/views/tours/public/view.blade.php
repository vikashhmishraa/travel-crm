...
<nav class="breadcrumb"> <a class="breadcrumb-item" href="{{ url('') }}">Home</a> <a class="breadcrumb-item" href="{{ url('trips') }}">Tours</a> <a class="breadcrumb-item" href="{{ url('trips?cat='.$row->tourcategoriesID) }}">{{ SiteHelpers::formatLookUp($row->tourcategoriesID,'tourcategoriesID','1:def_tour_categories:tourcategoriesID:tourcategoryname') }}</a> <span class="breadcrumb-item active">{{ $row->tour_name}}</span> </nav>
...
@endif	
        ...
        @endif " alt="{{$row->tour_name}}"><span>{{ $row->tour_name }}</span></h2><br><h4>{{ SiteHelpers::formatLookUp($row->tourcategoriesID,'tourcategoriesID','1:def_tour_categories:tourcategoriesID:tourcategoryname') }} - <small>{{ $row->total_days}} Days - {{ $row->total_nights}} Nights</small></h4> 
...
<div class="p-0 p-lg-2 d-lg-flex justify-content-start align-items-center">
                  ...
                  <p class="d-md-inline-block w-70 w-lg-20 my-0 mr-lg-1 font-weight-bold">
                    ...
                    <div class="progress w-30 w-lg-30 mb-2 mb-lg-0" data-toggle="tooltip" data-placement="top" title="30 spaces left in class">
                      <div class="progress-bar w-25 progress-bar-md bg-pink py-2" role="progressbar" aria-valuenow="{!! GeneralStatus::tourCapacity($td['tourID'] , $td['total_capacity'] ) !!}
...
{!! SiteHelpers::formatLookUp($row->policyandterms,'policyandterms','1:termsandconditions:tandcID:tandc') !!}
            </div>
        ...
        @foreach($similartours as $tour)
          <div class="card product-card overlay-hover">
            ...
            <div class="pos-relative">
              <img class="card-img-top img-fluid" src="uploads/images/{{ SiteHelpers::formatLookUp($tour,'tourID','1:tours:tourID:tourimage')}}" alt="{{ SiteHelpers::formatLookUp($tour,'tourID','1:tours:tourID:tour_name')}}">
            </div>
          </div>
          @endforeach
        </div>
