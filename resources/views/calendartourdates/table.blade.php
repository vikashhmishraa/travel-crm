...
<div class="box box-primary">
	...
	<div class="box-body"> 	
	 ...
	 <?php echo Form::open(array('url'=>'calendartourdates/delete/', 'class'=>'form-horizontal' ,'id' =>'MmbTable'  ,'data-parsley-validate'=>'' )) ;?>
			...
			<tr id="form-0" >
			  ...
			  @endif        
           		...
           		<?php foreach ($rowData as $row) : 
           			  $id = $row->tourdateID;
           		?>
                <tr class="editable" id="form-{{ $row->tourdateID }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids" name="ids[]" value="<?php echo $row->tourdateID ;?>" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->tourdateID }}" data-url="{{ url('calendartourdates/show/'.$id) }}"><i class="fa fa-plus " ></i></a></td>								
					@endif		
				 <td data-values="action" data-key="<?php echo $row->tourdateID ;?>"  >
					{!! AjaxHelpers::buttonAction('calendartourdates',$access,$id ,$setting) !!}
					{!! AjaxHelpers::buttonActionInline($row->tourdateID,'tourdateID') !!}	
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->tourdateID }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
