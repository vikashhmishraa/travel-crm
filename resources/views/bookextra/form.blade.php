...
<div class="box box-primary">
	...
	<div class="box-body"> 
@endif	
			{!! Form::open(array('url'=>'bookextra/save/'.SiteHelpers::encryptID($row['bookextraID']), 'class'=>'form-horizontal','files' => true , 'parsley-validate'=>'','novalidate'=>' ','id'=> 'bookextraFormAjax')) !!}
			<div class="col-md-12">
						<fieldset><legend> {{ Lang::get('core.bookextra') }}</legend>
				{!! Form::hidden('bookextraID', $row['bookextraID']) !!}
                            {!! Form::hidden('bookingID', app('request')->input('bookingID') ) !!}					
									  <div class="form-group  " >
										<label for="Extra Service" class=" control-label col-md-4 text-left">{{ Lang::get('core.bookextra') }}<span class="asterix"> * </span></label>
										<div class="col-md-7">
										  <select name='extraserviceID' rows='5' id='extraserviceID' class='select2 ' required  ></select> 
										 </div> 
									  </div> 
                                    <div class="form-group  " >
										<label for="Remarks" class=" control-label col-md-4 text-left"> {{ Lang::get('core.remarks') }}</label>
										<div class="col-md-7">
										  <textarea name='remarks' rows='4' id='remarks' class='form-control'  
...
$(document).ready(function() { 
		...
		$("#extraserviceID").jCombo("{!! url('bookextra/comboselect?filter=def_extra_services:extraserviceID:service_name&limit=WHERE:status:=:1') !!}",
		{  selected_value : '{{ $row["extraserviceID"] }}' });
	...
	$('input[type="checkbox"],input[type="radio"]').iCheck({
		...
		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("bookextra/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
