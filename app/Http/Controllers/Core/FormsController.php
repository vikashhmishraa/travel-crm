...
class FormsController extends Controller {
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			return Redirect::to('core/forms')->with('messagetext',\Lang::get('core.norecord'))->with('msgstatus','error');					
		}
	}	
   ...
   function postReorder( Request $request , $ids )
   {
    ...
    function convertForm( $id , $rows , $num )
    {
    	...
    	$remove = '<a href="#"  onclick="removeField(\''.$id.'\',\''.$field.'\'); return false;"><i class="fa fa-trash-o fa-lg"></i> Remove </a>';
    	switch ($type)
    	{
  			default;
				$form = '
				...
				<span class="pull-right"><a href="#" onclick="MmbModal(\''.$edit.'\')"><i class="fa fa-edit fa-lg "></i> Edit </a> | '.$remove.'</span>
				</div>';
				break;
			...
			case 'hidden';
				...
				<input type="hidden" name="sortlist['.$field.']" class="reorder" value="'.$num.'"  />
				<span class="pull-right"><a href="#" onclick="MmbModal(\''.$edit.'\')"><i class="fa fa-edit fa-lg "></i> Edit </a> | 
				'.$remove.'</span>
				</div>';
				break;				
    ...
    function getRebuild( $id )
    {
		...
		$forms .= '<div class="form-group  col-md-6" '.$attr.'>
	...
	function postProccess( Request $request , $formID )
	{
		...
		if ($validator->passes()) {
			...
			if($row->method =='table')
			{
				...
				\Mail::send('core.forms.templateform', $data, function ($message) use ($row) {
		    		$message->to($row->email)->subject('Submitted Form :  '. $row->name);
		    	});
		    	
                } else {
					    	$message = view('core.forms.templateform',$data);
